...
-> advantage: easy to upload : *read_pdf()* function
-> disadvantage: cleaning can be more difficult
...
[ADD SAMES CODES BUT USING TIDYTEXT]
...
The advantage of removing stop words is that it reduces the dimension of the number of features/terms to analyze so that the focus of the analysis is on terms that bring relevant informatio (i.e. dimension reduction method). 
...
Stemming simplifies tokens by reducing a word to its stem with simple rule based algorithm *token_wordstem()* function. Stemming reduces the size of a corpus'? vocabulary. 
...
text(log(frequency)~log(rank), 
     data = data.freq[1:4,],     # ADD LAST 4 WORDS ON THE DOTS OF THE PLOT
     label = feature, 
     pos = 4)
