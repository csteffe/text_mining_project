...
```{r start and stop, echo = FALSE, message = FALSE}
...
library(lexicon)
data.tk  %>%  tokens_replace(pattern = hash_lemmas$token, 
...
data.dfm <- dfm(data.tk)  # EST-CE QUE LE OUTPUT EST NORMAL ? CA ME SEMBLE CHELOU ??
View(data.dfm)
...
The following output shows the twenty terms that have the largest TF-IDF. [Interpret the first term]
...
The DTM represents the corpus by a matrix. Each row indicates a document ..

### Zipf's Law

The Zipf law shows the distribution of words used in a corpus. It says that most frequent terms are not specific one document since they are frequent in all documents. 
...
plot(frequency~rank, data = data.freq, pch = 20)

# Adds text on plot
text(frequency~rank, data = data.freq[1:4,], label = feature, pos=4)

```

The below plot displays the same plot but on a log-log-scale because of the large size of the corpus. On a log-log scale, the frequency versus rank gives a linear relation [EXPLAIN WHY]. 
...
# Illustration of the Zipf's law 
plot(log(frequency)~log(rank), data = data.freq, pch = 20)

# Adds text on plot
text(log(frequency)~log(rank), data = data.freq[1:4,], label = feature, pos = 4)
