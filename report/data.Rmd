...
data.tk2 <- data.tk1 %>%
            tokens_remove(c(stop_words$word,"chapter")) %>%
            tokens_tolower()
...
Lemmatization also simplifies tokens by generating a token dictionary for each token to reduce a vocabulary (i.e. set of types in a corpus) to its useful essence. For example, "started" was reduced to "start" and has thus "start" as a lemma. 
...
Steaming simplifies tokens by reducing a word to its stem with simple rule based algorithm *token_wordstem()* function. Stemming reduces the size of a vocabulary. 
...
```{r frequency, echo=FALSE, message=FALSE}
...
The Zipf's law shows the distribution of words used in a corpus. It says that most frequent terms are not specific to one document since they are frequent in all documents. [NON PAS DE IDF DANS Zip's Low seulement rank et frequency]

The below plot displays the term frequencies against their ranks on a non log-log scale. [PAS CE PLOT EN LOG-LOG]
...
ggplot(data.freq, aes(x=rank, y=frequency))+ 
  geom_point()+
  geom_label(data = data.freq %>%
              filter(rank <= 10), aes(label=feature))+
  ggtitle("The Zipf's law")

      
```

And the second plot is on a log-log scale, the frequency versus rank gives a linear relation [EXPLAIN WHY]. 
[La distribution au-dessus est exponentielle du coup en applicant le log elle devient linéaire.]
...
Both plots show that *women*, *female*, *data* and *found* are the most frequent terms of the corpus indicating that they are not specific to one chapter (i.e. document) but are frequent in all chapters. [NON PAS LA BONNE CONCLUSION]
