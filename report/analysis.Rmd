...
data.lsa <- textmodel_lsa(supervied_dfm, nd = 30) # reduce feature to 30 dimensions
...
The diagonal indicates the correct predictions of pages in their actual parts of the book [CORRECT ?] and around it we see the false predictions. For example, for part IV *Going to the Doctor*, 19 pages are correctly classified and four pages are incorrectly predicted.

The overall statistics demonstrates an overall accuracy (i.e. proportion of correct predictions) of 71.4% and balanced accuracy (i.e. average between sensitivity and specificity) for the classes over 70% as well, indicating that our model is good. The interesting fact here is that even after reducing the dimension to 30 of the DTM, we still obtain a high accuracy. 

The specificity shows that the proportion of true negative is very good. Indeed, predictions of pages in the wrong parts are often well achieved. For example, in parts V and VI it reaches 100%. On the other hand, the sensitivity shows that the proportion of true positive in four parts out of six are high whereas in parts IV and V is quite low, indicating that the model is not as successful as classifying their pages using the terms in these parts of the book. 
...
As seen previously, pages are not well classified in parts IV and V are not (respective sensitivity: 0.4333 and 0.5455) indicating that at some point in the previous steps something can be improved. It is possible that the splitting into the training and test sets divided the features in a manner that was not optimal for the classifier to better classify the pages in these two parts. Therefore, we proceed to improving the construction of the features.

[NEED TO ASK U A Q: I SET A SEED OTHERWISE PLOT CHANGES ALL THE TIME AND SO I DONT KNOW HOW TO INTERPRET: IT IS SUPPOSED TO SHOW FOR A CHOSEN DIMENSION CHOICE(nd.vac) THE ACCURCAY]
...
nd.vec <- c(2, 5, 25, 30, 50, 100, 1000, 2000)

acc.vec <- numeric(length(nd.vec))

for (j in 1:length(nd.vec)){
  
  data.lsa <- textmodel_lsa(data.dfm, nd = nd.vec[j])
...
Another method to verify whether the accuracy can be improved is to fit the model using the TF-IDF instead of the DTM. Here we reduce the dimension of the TF-IDF using the Latent Semantic Analysis (LSA) by targeting 30 dimensions as well.

[ADD ACCURACY COMPARISON ONCE RESULTS ARE SHOWN WITH PREVIOUS RESULTS USING DTM-> IS THERE IMPORVEMENT OR NOT ? Conclude]

```{r random forest using TF-IDF, echo=F, message=F, warning=F}

# Random forest using TF-IDF
data.tfidf <- dfm_tfidf(data.dfm)
data.lsa <- textmodel_lsa(data.tfidf, nd = 30)
df <- data.frame(Class = y, X = data.lsa$docs)
df.tr <- df[index.tr,]
df.te <- df[-index.tr,]
...
Here, we try to improve the classification of the model by adding more features [USUALLY THEY ARE DOCUMENT VARIABLES BUT HERE WE CLASSIFY PARTS SO DIFFERENT] to the [DTM OR TFIDF DEPENDING ON THE COMPARISON BETWEEN ACCURACY WITH DTM AND TFIDF ABOVE. IF BETTER ACCURACY WITH DTM THEN HERE WE USE DTM, IF OTHERWISE WE USE TFIDF HERE] DTM-LSA feature matrix or TF-IDF-LSA feature matrix [TO CHOOSE].

[STILL THINKING OF FEATURES TO ADD: THEORY SLIDE 5 FOR IDEAS]

[ADD COMMENT ON ACCURACY RESULT: IS THERE IMPORVMENT ? IF YES, THIS MEANS THERE WAS ROOM FOR IMPROVEMENT AND THAT ADDING X FEATURES REGARDING Y INCREASE ACCURACY BY ? + IF YES DO: sort(importance(data.fit), decreasing = TRUE) in a following chunk to see which of the features added are the most important = crucial in predicting in which part is a page, IF NOT THEN NO]

```{r classification improvement, echo=F, message=F, warning=F}
...
[ADD FEW LINES ON WHAT SVM DOES COMPARED TO RANDOM FOREST]
