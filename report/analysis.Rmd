...
kbl(head(hash_sentiment_jockers_rinker, 5), 
...
kbl(head(hash_valence_shifters, 5), 
...
sent_bychap <- sentiment_by(text) %>%
               arrange(element_id) %>%
               rename(Chapter = element_id) %>%
               mutate(document = paste("chapter", Chapter, sep = " ")) %>%
               relocate(document,.before = Chapter) %>% 
               select(!(Chapter))
...
sent_bychap %>% 
  ggplot(aes(x = reorder(document,ave_sentiment), y = ave_sentiment)) + 
