...
Looking at the complexity of the book, *Invisible Women* is built of 16 chapters divided in six parts and tells its story over 280 pages. The length of the document without any cleaning is 97'382. With the cleaning process we keep less than 40% of the tokens present in the document which represents a length of 36'160. 
...
data.sent <- tokens_lookup(data.tk4, 
             ...
             mutate(ord = as.numeric(str_sub(document, start = 5L)))

# Plots the above tibble
ggplotly(ggplot(data.sent,
                aes( x = reorder(document, -ord), 
                     y = count, 
                     fill = term, 
         ...
         coord_flip() + 
         theme_minimal() +
         scale_fill_manual(values = c("dodgerblue","darkseagreen3")) +
...
After building the LDA object, we look at the top three terms per topic and the top three topics per document. The following output shows that *public*, *sexual* and *toilet* are the top three words of topic 1 and that topic 6 is the most frequent topic. 
...
Before generating the centroids, we extract words in each document and for these words, we extract the word vectors and make a matrix. Then, we average all these vectors. The centroid method averages the word representations over the number of documents.

In the below plot, we can see that the chapters 2-13, 1-6 and 8-14 are very close. It means that they look similar because they use terms that are similar and co-occur. In addition, chapters 3, 10 and 12 are less similar to the rest of the corpus for these reasons.
...
plot(data.detfidf, type='n',  xlab="Dim 1", ylab="Dim 2", main="Document Embedding: Centroids (TFIDF)")

text(x=data.detfidf[,1], y=data.detfidf[,2], labels=rownames(data_tfidf))

```
Similarly to the previous centroïd method, chapter 10 is by far the least similar chapter in comparison to the other ones. Due to the fact that we use the TF-IDF, the chapters are a bit more spread. Indeed, by definition terms are more specific to each chapter. However, chapters 5 and 8 are particularly similar which was not the case previously.
...
Interestingly, chapter 10 is now at the center and very close to many chapters which indicates that according to the RWMD measure the cost of transforming words from chapter 10 to words of other chapters is quite low. This finding is in opposition with results from Centroïd methods.
...
The inverted Jaccard dissimilarity matrix shows that there are two main clusters. One cluster groups chapters 15 *Who Will Rebuild* and 16 *It's Not The Disaster That Kills You* and the other one groups the rest of the book. Moreover, inside the second larger cluster, we see that there are two sub-clusters. From the dendogram, we understand that chapters 10 *The Drugs Don't Work* and 11 *Yentl Syndrom* are the most similar ones as their distance is the smallest (~0.76).

On the contrary, chapters 4 and 14, respectively *The Myth of Meritocracy* and *Women’s Rights are Human Rights*, are the furthest apart. When questioning ourselves, we note that chapter 10 and 11 are pretty far apart from the others. When observing the title chapters, we observed that it is, along with chapter 5 *The Henri Higgins Effect*, the only chapters to touch on very particular subjects.
...
For interpretation purposes, we extract the ten words that are the most frequent in each cluster to identify a common denominator between chapters. According to the most used terms in each cluster :

* Cluster 1 groups terms about public transports or spaces
* Cluster 2 about medical or technological trials 
* Clusters 3 to 6 are composed of only one chapter and therefore group terms according to their respective chapters' vocabulary.
...
For interpretation purposes, we extract the ten most frequent terms of each cluster to identify a common denominator between chapters. According to the most used terms in each cluster:

* Cluster 1's chapters share terms specific to public transport or spaces and violence
* Cluster 2's chapters use terms regarding households, families and economics
* Cluster 3 reveals a political vocabulary
* Cluster 4 shows terms related to technology
* Cluster 6 displays terms about agriculture and finally 
* Cluster 6 indicates a medical vocabulary
...
# Makes a co-occurrence on a window of 30
...
#remove women
data.fcm.mat <- data.fcm.mat%>% filter(Var1 != "woman" & Var2 != "woman")
...
For example *female* and *male* are the most similar according to the co-occurrence dissimilarity matrix.
...
# Cluster removing women ( first row and first column)
data.hc <- hclust(as.dist(data.inv_occ[-1,-1]))
...
Random Forest is a supervised learning algorithm used in regression and classification problem. It consists of building many decisions trees that will be averaged to make a final prediction. Here we will do classification for the 6 parts of the book :  
...
index.tr <- sample(size = round(0.8*length(y)), 
                   x = c(1:length(y)), 
                   replace = FALSE)
...
rf.pred <- predict(rf.model, df.te)

confusionMatrix(rf.pred, df.te$Class)
...
```{r classification improvement, eval=FALSE, message=FALSE, warning=FALSE}
set.seed(235) # for reproducibility of the results
