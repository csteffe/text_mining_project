...
neg_pos <- data.sent %>%
           filter(term == "neg_positive")

neg_neg <- data.sent %>%
           filter(term == "neg_negative")
...
data.lsa <- textmodel_lsa(data.dfm2, 
...
y <- as.factor(data$part)

# dimension reduction with LSA 
# reduce feature to 30 terms
data.lsa <- textmodel_lsa(data.dfm2, nd = 30) 
...
index.tr <- sample(size=round(0.7*length(y)), x=c(1:length(y)), replace=FALSE)
df.tr <- df[index.tr,]
df.te <- df[-index.tr,]
