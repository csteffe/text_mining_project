...
The goal of the supervised analysis is to re-classify the chapters of the book using their terms. To do so, we proceed to a machine learning approach consisting of splitting the corpus into a training and a test sets. Then, we train the classifiers on the training set and finally, we select the best classifiers from the results on the test set.
...
From the results of LSA, we proceed to fir the classifiers on the training set and predict the results on the test set. The below results show first the overall quality of the predictions of the random forest classifier on the confusion matrix. We observe that 
...
# Split training set 40% and test set 20%
