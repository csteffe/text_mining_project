...
much more no similarity (darker blue squares) between terms used in different chapters than the previous one. However, chapter 10 and 11 are still shown as sharing more similarities.
...
For interpretation purposes, we extract the ten most frequent terms of each cluster to identify a common denominator between chapters. According to the most used terms in each cluster, cluster 1 shows terms specific to transport and travel, cluster 2's chapters use terms that are not necessarily specific to one topic, cluster 3 reveals terms related to technology, cluster 4 indicates a medical vocabulary, cluster 5 displays terms related to households and economics and finally, cluster 6 shows politics-related terms. 
...
The below output shows, for the six longest chapters, the highest conditional probabilities $\theta$ for each topic. For example, chapter 2 *Gender Neutral With Urinals* mainly talks about topic 1 and 10 (not in the same proportion though).
